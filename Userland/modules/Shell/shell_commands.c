...
void run_in_foreground(task_fn_t fn, void *arg)
{
    ...
    int a = fd_bind_std(pid, STDIN, STDIN);
...
shell_cmd_t shell_commands[] = {
    {"help", 0, 0, "Mostrar comandos disponibles", "help", 0, (task_fn_t)cmd_help_run},
...
int command_switch(char **line)
{
    for (int i = 0; i < shell_commands_count; i++)
    {
        ...
        if (!strcmp(line[0], c->name))
        {

            run_in_foreground((task_fn_t)c->run, line[1]);
            /*
            task_fn_t fn = (task_fn_t)c->run;
            task_fn_t fn_2 = (task_fn_t)cmd_help_run;
            if (fn == fn_2)
            {
                // fprintf(3, "%d %d \n", fn, fn_2);
                run_in_foreground(fn, line[1]);
            }
            */

            // run_in_foreground((task_fn_t)cmd_help_run, line[1]);
            /*
            char *argv_raw = line[1];

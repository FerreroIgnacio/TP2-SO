...
uint8_t BackFb [100000000]; // CAMBIAR POR MALLOC
...
void clearPlayer(player_t p) {
    fbDrawCircle(newFb, BG_COLOR, p.x, p.y, PLAYER_RADIUS);
    point_t Leye, Reye;
    getPlayerEyePos(p, &Leye, &Reye);
    fbDrawCircle(newFb, BG_COLOR, Leye.x, Leye.y, PLAYER_EYES_RADIUS);
    fbDrawCircle(newFb, BG_COLOR, Reye.x, Reye.y, PLAYER_EYES_RADIUS);
}
void drawPlayer(player_t p) {
    fbDrawCircle(newFb, p.color, p.x, p.y, PLAYER_RADIUS);
    point_t Leye, Reye;
    getPlayerEyePos(p, &Leye, &Reye);
    fbDrawCircle(newFb, 0xFFFFFF, Leye.x, Leye.y, PLAYER_EYES_RADIUS);
    fbDrawCircle(newFb, 0xFFFFFF, Reye.x, Reye.y, PLAYER_EYES_RADIUS);

    point_t Lpupil, Rpupil;
    getPlayerPupilPos(p, &Lpupil, &Rpupil);
    fbDrawCircle(newFb, 0x000000, Lpupil.x, Lpupil.y, PLAYER_PUPIL_RADIUS);
    fbDrawCircle(newFb, 0x000000, Rpupil.x, Rpupil.y, PLAYER_PUPIL_RADIUS);
}
...
void drawHole(hole_t holeToDraw) {
    fbDrawCircle(newFb, holeToDraw.color, holeToDraw.x, holeToDraw.y, holeToDraw.radius);
}
...
int main() {
    ...
    fbFill(BackFb, BG_COLOR);
    hole_t drawables[] = {target, *(hole_t*)&pit, *(hole_t*)&mount};
    for (int i = 0; i < sizeof(drawables) / sizeof(drawables[0]); i++) {
        fbDrawCircle(BackFb, drawables[i].color, drawables[i].x, drawables[i].y, drawables[i].radius);
        fbDrawCircle(newFb, drawables[i].color, drawables[i].x, drawables[i].y, drawables[i].radius);
    }
    uint64_t lastTick = 0;
    drawPlayer(p1);
    drawPlayer(p2);

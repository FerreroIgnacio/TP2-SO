...
static char getAscii(uint8_t makecode){
    ...
    if (altgrPressed) return makeCodeToAscii_altgr[makecode];
    if (shiftPressed && !capsLockOn) return makeCodeToAscii_shift[makecode];
    if (!shiftPressed && capsLockOn) return makeCodeToAscii_shift[makecode];
...
void keyPressedAction(uint8_t makecode, registers_t * regs) {
    ...
    if (extended){                                                      // makecode extendido (E0)
        if (makecode & 0x80) {
        ...
        if (makecode & 0x80) {                                          // makecode normal (sin E0)
